...
void print_out(char *output_template, void *value_location, size_t byte_count) {
  ...
  ssize_t result_length = snprintf(output_buffer, BUF_SIZE, output_template,
                                   byte_count == sizeof(uint64_t)
                                       ? *(uint64_t *)value_location
...
void print_block(char *memory_location) {
  int position;
  for (position = 0; position < BLOCK_SIZE - sizeof(struct header);
       ++position) {
...
void initialize_block(struct header *block_header, uint64_t total_bytes,
                      struct header *successor, int padding_value) {
...
int main(void) {
  ...
  struct header *secondary_header =
      (struct header *)(allocated_region + BLOCK_SIZE);
  ...
  print_out("First block size: %lu\n", &primary_header->size,
            sizeof(primary_header->size));
  print_out("Second block size: %lu\n", &secondary_header->size,
            sizeof(secondary_header->size));
  print_out("First block next: %p\n", &primary_header->next,
            sizeof(primary_header->next));
  print_out("Second block next: %p\n", &secondary_header->next,
            sizeof(secondary_header->next));
